...
for example in available_examples[:60]:
    # example_data_list.append({"tasks": example.split('\n')[0], "steps": '.'.join(example.split('\n')[1:])})
    task = example.split('\n')[0]
    # steps = '.'.join(example.split('\n')[1:])
    if args.intervene_type == "conf":
        ...
        if isLocationStep:
            ...
            # directly prepend the navigation step before the original steps; Note that here is a small step index issue
            intervened_program_str = intervened_program_str + new_step_str + '\n'.join(example.split('\n')[1:])
