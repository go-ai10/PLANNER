...
class IntentProcessor(DataProcessor):
    ...
    def get_example_from_tensor_dict(self, tensor_dict):
        ...
        return InputExample(
            ...
            tensor_dict["task"].numpy().decode("utf-8"),
            tensor_dict["steps"].numpy().decode("utf-8"),
            str(tensor_dict["label"].numpy()),
        )

    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "atomic_train.tsv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "atomic_dev.tsv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "atomic_test.tsv")), "test")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, percentage=100):
    ...
    processor = IntentProcessor() #processors[task]()
    ...
    if task in ["mnli", "qnli", "wnli", "rte", "sts", "mrpc", "qqp", "sts-b"]:
        dataset = TensorDataset(all_input_ids[:sampled_data_end], all_attention_mask[:sampled_data_end], all_token_type_ids[:sampled_data_end], all_labels[:sampled_data_end])
    else:
        dataset = TensorDataset(all_input_ids[:sampled_data_end], all_attention_mask[:sampled_data_end], all_token_type_ids[:sampled_data_end], all_labels[:sampled_data_end])
    return dataset
...
def main():
    ...
    processor = IntentProcessor() #processors[args.task_name]()
    ...
    if args.unifylangvis:
        ...
        model_langvis = RewardActionFunction(
                            num_classes=task_num_classes,
                            loss_fn=loss_function,
                            prompt_feature_dim=512,
                            program_feature_dim=512,
